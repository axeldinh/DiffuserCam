...
This is done using the non negative Huber normed problem.

To use it either use:
```bash
python scripts/huber_myDataset.py --data myDataset --save
```
to compute the reconstructions and evaluate them, or:
```bash
python scripts/huber_myDataset.py --data myDataset --load
```
...
def huber_myDataset(data, n_iter, gray, save, load):
    ...
    save_path = "results_huber_myDataset"
